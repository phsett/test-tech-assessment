...
export class ReservationPage {
  ...
  async reservationValidationErrors() {
    await expect(
      this.page.getByText("must not be empty").first()
    ).toBeVisible();
    await expect(
      this.page.getByText("Lastname should not be blank")
    ).toBeVisible();
    await expect(
      this.page.getByText("Firstname should not be blank")
    ).toBeVisible();
    await expect(this.page.getByText("must not be empty").nth(1)).toBeVisible();
    await expect(
      this.page.getByText("size must be between 11 and 21")
    ).toBeVisible();
    await expect(
      this.page.getByText("size must be between 3 and 30")
    ).toBeVisible();
    await expect(
      this.page.getByText("size must be between 3 and 18")
    ).toBeVisible();
  }
